...
static const char *TAG = "Ml307Transport";


Ml307Transport::Ml307Transport(Ml307AtModem& modem, int tcp_id) : modem_(modem), tcp_id_(tcp_id) {
        ...
        if (command == "MIPOPEN" && arguments.size() == 2) {
            if (arguments[0].int_value == tcp_id_) {
                if (arguments[1].int_value == 0) {
                    connected_ = true;
                    xEventGroupClearBits(event_group_handle_, ML307_TRANSPORT_DISCONNECTED | ML307_TRANSPORT_ERROR);
                    xEventGroupSetBits(event_group_handle_, ML307_TRANSPORT_CONNECTED);
                } else {
                    connected_ = false;
                    xEventGroupSetBits(event_group_handle_, ML307_TRANSPORT_ERROR);
                }
            }
        } else if (command == "MIPCLOSE" && arguments.size() == 1) {
            if (arguments[0].int_value == tcp_id_) {
                connected_ = false;
                xEventGroupSetBits(event_group_handle_, ML307_TRANSPORT_DISCONNECTED);
            }
        } else if (command == "MIPSEND" && arguments.size() == 2) {
            if (arguments[0].int_value == tcp_id_) {
                xEventGroupSetBits(event_group_handle_, ML307_TRANSPORT_SEND_COMPLETE);
            }
            ...
            if (arguments[1].int_value == tcp_id_) {
                if (arguments[0].string_value == "rtcp") {
                    ...
                    xEventGroupSetBits(event_group_handle_, ML307_TRANSPORT_RECEIVE);
            ...
            if (arguments[0].int_value == tcp_id_) {
                ...
                xEventGroupSetBits(event_group_handle_, ML307_TRANSPORT_INITIALIZED);
            }
...
Ml307Transport::~Ml307Transport() {
    modem_.UnregisterCommandResponseCallback(command_callback_it_);
}

bool Ml307Transport::Connect(const char* host, int port) {
    char command[64];

    // Clear bits
    xEventGroupClearBits(event_group_handle_, ML307_TRANSPORT_CONNECTED | ML307_TRANSPORT_DISCONNECTED | ML307_TRANSPORT_ERROR);

    // 检查这个 id 是否已经连接
    sprintf(command, "AT+MIPSTATE=%d", tcp_id_);
    modem_.Command(command);
    auto bits = xEventGroupWaitBits(event_group_handle_, ML307_TRANSPORT_INITIALIZED, pdTRUE, pdFALSE, pdMS_TO_TICKS(TRANSPORT_CONNECT_TIMEOUT_MS));
    if (!(bits & ML307_TRANSPORT_INITIALIZED)) {
    ...
    // 等待连接完成
    bits = xEventGroupWaitBits(event_group_handle_, ML307_TRANSPORT_CONNECTED | ML307_TRANSPORT_ERROR, pdTRUE, pdFALSE, TRANSPORT_CONNECT_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (bits & ML307_TRANSPORT_ERROR) {
...
void Ml307Transport::Disconnect() {
    if (!connected_) {
        return;
    }
    connected_ = false;
...
int Ml307Transport::Send(const char* data, size_t length) {
    ...
    while (total_sent < length) {
        ...
        auto bits = xEventGroupWaitBits(event_group_handle_, ML307_TRANSPORT_SEND_COMPLETE, pdTRUE, pdFALSE, pdMS_TO_TICKS(TRANSPORT_CONNECT_TIMEOUT_MS));
        if (!(bits & ML307_TRANSPORT_SEND_COMPLETE)) {
...
int Ml307Transport::Receive(char* buffer, size_t bufferSize) {
    while (rx_buffer_.empty()) {
        auto bits = xEventGroupWaitBits(event_group_handle_, ML307_TRANSPORT_RECEIVE | ML307_TRANSPORT_DISCONNECTED, pdTRUE, pdFALSE, portMAX_DELAY);
        if (bits & ML307_TRANSPORT_DISCONNECTED) {
            return 0;
        }
        if (!(bits & ML307_TRANSPORT_RECEIVE)) {
